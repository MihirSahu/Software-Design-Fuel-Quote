...
export async function calculateTotalAmount(gallonsRequested: number, isTexas: boolean): Promise<{ suggestedPrice: string, totalAmount: string }> {
    const currentPrice = new Decimal(1.50); // Static current price per gallon
    const locationFactor = new Decimal(isTexas ? 0.02 : 0.04);
    
    const hasHistory = await hasClientRequestedBefore();
    const rateHistoryFactor = new Decimal(hasHistory ? 0.01 : 0.00);

    const gallonsRequestedFactor = new Decimal(gallonsRequested > 1000 ? 0.02 : 0.03);
    const companyProfitFactor = new Decimal(0.10);

    const margin = currentPrice.mul(locationFactor.minus(rateHistoryFactor).plus(gallonsRequestedFactor).plus(companyProfitFactor));

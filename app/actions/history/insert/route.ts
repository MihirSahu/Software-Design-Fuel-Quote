...
export async function POST(request: Request) {
  ...
  const insertHistory = await supabase
    .from('history')
    .insert({ history_id: historyId['data'][0]['history_id'], gallons_requested: gallonsRequested, delivery_address: deliveryAddress, delivery_date: deliveryDate, price: pricePerGallon, amount_due: totalAmountDue, request_date: requestDate})
